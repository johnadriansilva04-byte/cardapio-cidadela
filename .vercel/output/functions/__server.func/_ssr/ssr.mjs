	...
	globalThis.addEventListener("error", (event) => {
		...
		if (target && target.tagName) {
			...
			if ([
				...
				let src;
				if (tagName === "link") src = element.href;
				else if (tagName === "script") src = element.src;
				else if (tagName === "img") src = element.src;
				else if (tagName === "iframe") src = element.src;
				if (src) {
...
async function getServerEntry() {
	if (!serverEntryPromise) serverEntryPromise = import("./server-DZBEtXF9.mjs").then((m) => m.default ?? m);
