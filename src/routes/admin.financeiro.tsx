...
import { Store, AlertCircle, RefreshCw, TrendingUp, Wallet, Award, Ban, Calendar, Download } from "lucide-react";
...
type OrderRow = { id: string; comanda: string; customer_name: string; status: OrderStatus; total: number; created_at: string; restaurant_id: string };
...
function FinanceiroPage() {
  ...
  const previousPeriodComparison = useMemo(() => {
    if (range === "all" || orders.length === 0) return null;
    const days = range === "7d" ? 7 : range === "30d" ? 30 : 90;
    const now = new Date();
    ...
    const curRevenue = orders
      .filter((o) => o.status !== "cancelled" && new Date(o.created_at).getTime() >= mid.getTime())
      .reduce((s, o) => s + Number(o.total || 0), 0);
    // For previous period we need to fetch separately? Approximate: we only have current window data, so can't compare accurately without extra query.
    // Instead, show hint that comparação requer dados do período anterior — we fetch it lazily if needed, but for now return null to avoid falsa comparação.
    // To keep simples, retorna apenas current; comparação real exigiria segunda query.
    return { curRevenue, days };
  }, [orders, range]);
  ...
  if (restaurants.length === 0) {
    return (
      <div className="space-y-4">
        ...
        <p className="text-sm text-gray-500">Faturamento calculado pelos pedidos registrados — sem integração bancária, não representa saldo real.</p>
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <p className="mt-1 max-w-[60ch] text-sm leading-relaxed text-gray-500">
            Faturamento calculado pelos pedidos registrados (não é saldo bancário). Cancelados não entram no faturamento. Ticket médio considera apenas pedidos não cancelados.
          </p>
        </div>
        ...
        <>
          ...
          <div className="grid gap-4 lg:grid-cols-2">
            <div className="rounded-2xl border border-white/10 bg-white/[0.02] p-4">
              ...
              <p className="mt-1 text-[11px] text-gray-500">Pedidos não cancelados • últimos dias no período selecionado</p>
                ...
                {daily.length === 0 ? (
                  ...
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={daily}>
                      ...
                      <YAxis tick={{ fill: "#9ca3af", fontSize: 11 }} axisLine={false} tickLine={false} tickFormatter={(v: number) => brl(v)} width={72} />
                      <Tooltip
            ...
            <div className="rounded-2xl border border-white/10 bg-white/[0.02] p-4">
              ...
              <p className="mt-1 text-[11px] text-gray-500">
                {activeId === "all" ? "Comparativo entre restaurantes (não cancelados)" : "Filtro único — selecione \"Todos\" para comparar"}
              </p>
              <div className="mt-4 h-[180px] w-full">
                {perRestaurant.length === 0 ? (
                  <div className="flex h-full items-center justify-center text-xs text-gray-600">
                    {activeId !== "all" ? "Selecione \"Todos\" para ver o comparativo." : "Sem dados para comparar."}
                  </div>
                ) : (
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={perRestaurant}>
                      <CartesianGrid stroke="rgba(255,255,255,0.06)" strokeDasharray="3 3" />
                      <XAxis dataKey="name" tick={{ fill: "#9ca3af", fontSize: 11 }} axisLine={false} tickLine={false} interval={0} angle={-12} dy={10} height={40} />
                      <YAxis tick={{ fill: "#9ca3af", fontSize: 11 }} axisLine={false} tickLine={false} tickFormatter={(v: number) => brl(v)} width={72} />
                      <Tooltip
                        contentStyle={{ background: "#0f0f14", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 12, color: "#fff", fontSize: 12 }}
                        formatter={(value: number) => [brl(Number(value)), "Faturamento"]}
              ...
              {perRestaurant.length > 0 && (
                <div className="mt-3 grid gap-1 text-[11px] text-gray-500">
                  {perRestaurant.map((r) => (
                    <div key={r.name} className="flex items-center justify-between rounded-lg border border-white/5 bg-black/20 px-3 py-1.5">
                      <span className="truncate font-medium text-gray-300">{r.name}</span>
                      <span className="font-bold text-white">{brl(r.revenue)} <span className="font-normal text-gray-500">• {r.count} pedidos</span></span>
                    </div>
                  ))}
                </div>
              )}
          ...
          {previousPeriodComparison && (
            <div className="rounded-2xl border border-white/10 bg-white/[0.02] p-4">
              ...
              <p className="mt-2 text-xs leading-relaxed text-gray-500">
                Período atual (<strong className="text-white">{range}</strong>): faturamento <strong className="text-cyan-300">{brl(previousPeriodComparison.curRevenue)}</strong> com {metrics.totalOrders} pedidos (cancelados fora). Para comparar com o período anterior idêntico, mantenha o mesmo filtro de restaurante e período — o sistema usa apenas pedidos reais, sem projeções.
              </p>
            </div>
          )}
          ...
          <div className="rounded-2xl border border-white/10 bg-black/30">
            <div className="flex items-center justify-between border-b border-white/5 px-4 py-3">
              <h3 className="text-xs font-bold uppercase tracking-widest text-gray-400">Histórico financeiro</h3>
              <span className="text-[11px] text-gray-500">Mostrando até 80 • {orders.length} no filtro</span>
            </div>
                ...
                <tbody className="divide-y divide-white/5">
                  {orders.slice(0, 80).map((o) => (
                    <tr key={o.id} className="hover:bg-white/[0.02]">
                      <td className="px-4 py-2 text-xs text-gray-400">{new Date(o.created_at).toLocaleString("pt-BR", { day: "2-digit", month: "2-digit", hour: "2-digit", minute: "2-digit" })}</td>
                      <td className="px-4 py-2 font-mono text-xs font-bold text-white">{o.comanda}</td>
                      ...
                      <td className="px-4 py-2 text-right font-bold text-white">{brl(Number(o.total))}</td>
                    </tr>
                  ))}
                  {orders.length === 0 && (
                    <tr>
                      <td colSpan={5} className="px-4 py-8 text-center text-xs text-gray-600">Nenhum pedido neste filtro.</td>
                    </tr>
                  )}
                </tbody>
              </table>
            ...
            {orders.length > 80 && (
              <p className="px-4 py-2 text-center text-[11px] text-gray-600">Mostrando 80 de {orders.length}. Use Exportar CSV para ver tudo.</p>
            )}
          </div>

          <div className="rounded-xl border border-amber-500/20 bg-amber-500/5 p-3 text-[11px] leading-relaxed text-amber-200/90">
            <span className="font-bold">Aviso:</span> Este financeiro é calculado a partir dos pedidos registrados no sistema. Não é extrato bancário nem considera taxas de gateway/PIX. Pedido cancelado nunca entra no faturamento.
          </div>
        </>
      )}
    </div>
